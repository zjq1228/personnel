...
<head>
    ...
    <script type="text/javascript">
        ...
        //展示
        function search() {
            $.post("<%=request.getContextPath()%>/notice/show",
                $("#fm").serialize(),
                function (data) {
                    if (data.code != 200) {
                        alert(data.msg);
        ...
        function add() {
            location.href = "<%=request.getContextPath()%>/notice/toInsert"
        }
        //删除
        function del() {
            selectedValue();
            var index = layer.load(0, {shade: 0.3}, {time: 10 * 1000});
            $.post("<%=request.getContextPath()%>/notice/deleteNotice",
                ...
                function (data) {
                    ...
                    if (data.code == "200") {
                        layer.msg(data.msg,
                            {icon: 6, time: 2000},
                            function () {
                                window.location.href = "<%=request.getContextPath()%>/notice/toShow"
                            });
                        return;
                    }
                    layer.msg(data.msg);
        ...
        function upd(id) {
            window.location.href = "<%=request.getContextPath()%>/notice/toUpdate?id=" + id;
        }
        ...
        function see(id){
            layer.open({
                ...
                shade: 0.8,
                area: ['380px', '90%'],
                content: '<%=request.getContextPath()%>/notice/toSee?id=' + id
            });
        }
...
<form id="fm">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
            ...
            <td class="main_locbg font2"><img src="${pagePath}/res/images/pointer.gif">&nbsp;当前位置：公告管理&nbsp;&gt;&nbsp;公告查询</td>
    ...
    <tr valign="top">
        <td height="30">
            <table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
                <tr>
                    <td class="fftd">
                        <form name="noticeform" method="post" id="noticeform" action="${pagePath}/notice/show">
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td class="font3">公告标题：<input type="text" name="title" >  <input type="button" value="搜索" onclick="selectBy()" /> <input type="reset" value="清空" /> <input onclick="del()" type="button" value="删除" />
                                    </td>
                                </tr>
                            </table>
                        </form>
